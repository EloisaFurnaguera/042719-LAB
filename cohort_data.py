...
def unique_houses(filename):
    ...
    for line in cohort_list: #por cada linea
        line = line.rstrip() #le borra espacio a cada linea sin guardar nada
        split_cohort_line = line.split("|") # cada linea la separa donde haya la divison
        if split_cohort_line[2]: #si pasas por indice 2 y hay algo
            houses.add(split_cohort_line[2])#agrega el indice 2 al set
...
def all_students_tuple_list(filename):
    ...
    for line in cohort_data:
        line= line.rstrip()
        split_cohort_line = line.split("|")
        if split_cohort_line [-1]== "I":
            pass
        elif split_cohort_line [-1]== "G":
            pass
        else:
            student_name = split_cohort_line[0] + " " + split_cohort_line[1]
            student_list.append((student_name, split_cohort_line[2], split_cohort_line[3], split_cohort_line[4]))
